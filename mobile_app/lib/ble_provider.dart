...
// ★★★★★ アイソレート関数を修正 ★★★★★
Future<List<SensorDataPoint>> _decompressAndParseIsolate(
  ...
  final lastTimestamp = byteData.getUint32(
      pointSize * (numPoints - 1) + (pointSize - 4), Endian.little);
  ...
  for (int i = 0; i < numPoints; i++) {
    ...
    for (int ch = 0; ch < numEegChannels; ch++) {
      eegs.add(byteData.getUint16(offset + (ch * 2), Endian.little));
    }
    final point = SensorDataPoint(
      eegValues: eegs,
      timestamp: espBootTime.add(Duration(
          microseconds:
              byteData.getUint32(offset + (pointSize - 4), Endian.little))),
    );
    newPoints.add(point);
  }
  return newPoints;
...
String get _serverUploadUrl => "$_baseUrl/upload";
...
class BleProvider with ChangeNotifier {
  ...
  int get channelCount =>
      _dataBuffer.isNotEmpty ? _dataBuffer.first.eegValues.length : 0;
      ...
      for (var service in services) {
        if (service.uuid == serviceUuid) {
          ...
          for (var characteristic in service.characteristics) {
            if (characteristic.uuid == characteristicUuidTx) {
              tx = characteristic;
            }
            if (characteristic.uuid == characteristicUuidRx) {
              rx = characteristic;
            }
          }
          if (tx != null && rx != null) {
            print("TX and RX characteristics found.");
    ...
    if (_expectedPacketSize != -1 &&
      ...
      try {
        final List<SensorDataPoint> newPoints =
            await compute(_decompressAndParseIsolate, compressedData);
  ...
  Future<void> _sendDataToServer(Uint8List compressedData) async {
    try {
      final url = Uri.parse(_serverUploadUrl);
      ...
      print("Data packet sent to server: $url");
    } catch (e) {
  ...
  Future<void> _sendAck() async {
    if (_rxCharacteristic != null) {
      try {
        await _rxCharacteristic!.write([0x01], withoutResponse: true);
      } catch (e) {
