...
本プロジェクトは、ユーザー一人ひとりが自身の「満足する（期待する）状態」を自覚し，その状態へとスムーズに進んでいくことを能動的に支援するアプリケーションを開発・提供します。その中核となるのは、ユーザーの「現実、現在の状態」と理想との距離を常に評価し、その障壁をエージェントが自動で打破する仕組みです。さらに、生体信号を活用して言語化されない欲求まで汲み取り、その過程で得られる良質なデータを社会の様々な分野で活用可能なビッグデータHubとして構築することを目指します。

## 目的
このプロジェクトが目指す最終的な到達点は以下の2つです。

個人の自己実現と満足度の最大化:
ソフトウェアの力によって、あらゆるユーザーが自身の「満足する（期待する）状態」へ向かう過程で直面するハードルや障壁を乗り越え、効率的に目標を達成できる社会を実現します。

実生活に根差したビッグデータHubの構築と社会貢献:
ユーザーの行動と評価から得られる「良質な教師ありデータ」を大規模に蓄積・提供することで、新たな学術研究や企業の潜在的ニーズの発見を促進し、実社会に貢献する情報基盤を構築します。

## 目標
1. ユーザー支援アプリケーションの開発と提供
ユーザーが満足する方向へ進むことを支援するため、以下の機能を持つアプリケーションを開発し、提供します。
状態分析: ユーザーの「現実、現在の状態」を正確に分析・可視化する。
期待値の定義: ユーザーとの対話を通じて、個々の「満足する（期待する）状態」を明確に定義（ラベリング）する。
進捗評価: ユーザーがとった進路やエージェントのアクションによって、理想と現実の距離がどのように変化したかをユーザー自身に評価してもらう（ラベリング）。
障壁の打破: ユーザーにとって「満足する（期待する）状態」から離れてしまう、あるいはハードルとなる作業や判断をエージェントが自動で処理し、ユーザーの行動をサポートする。

2. 生体信号を活用した潜在的欲求の把握
言語化のハードルを越え、より深いレベルでユーザーを理解するための技術を確立します。
データ統合: 生体信号を、ユーザーが置かれたシーン（状況）と組み合わせることで、コンテキストに応じたユーザーの状態を推定する。
潜在欲求の抽出: 明示的な操作や言語化に至らない欲求をデータから拾い上げ、よりパーソナライズされた，言語や特別なアクションを介さないサポートの実現に繋げる。

3. 良質な教師ありデータの蓄積と活用プラットフォームの構築
アプリケーションを通じて得られるデータを、価値ある資産として社会に還元する仕組みを構築します。
データベース構築: ユーザーの「状態」「行動」「評価」のサイクルから得られる、質の高い教師ありデータを体系的にDBへ蓄積する。
データ活用の推進: 蓄積されたビッグデータを、学術研究や企業の潜在的ニーズの発見に役立てるための分析環境やAPIを提供する。実生活で実用的なHubとしての役割を担うプラットフォームを構築する。
...
-----
# サーバーサイド設計
拡張性と保守性を最大限に高めるサーバーサイドの設計．

-----

## 🏛️ システムアーキテクチャ設計

各独立したサービス（モジュール）群でAPIやメッセージキューを介して通信．
サービスごとに最適な言語（TypeScript/Python）や技術を選択．
...
### 5\. Strategist Service (計画立案・実行)
  ...
  * **処理フロー**:
    ...
    3.  各タスクを実行するために、定義されたツールを呼び出す。
          * **ツール連携**: Pythonスクリプトを**サブプロセスとして呼び出す**か、あるいは各ツールを**個別のFastAPIサービスとして起動**し、HTTPリクエストで呼び出す。この方法なら、TypeScriptからPythonのツールをクリーンに利用できます。
